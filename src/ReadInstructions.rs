...
fn readEvent(event: &serde_json::Value) -> EventID {
    let event_struct: &serde_json::Value = event.get("event").expect("Event does not contain an Event metadata!");
    EventID {
...
fn readCond(cond: &serde_json::Value) -> CondID {
    let cond_struct: &serde_json::Value = cond.get("cond").expect("Condition does not contain an Cond metadata!");
    CondID {
...
fn readFunc(event: &serde_json::Value) -> FuncTypes {
    let func: &serde_json::Value = event.get("func").expect("ReadInstructions: Cannot parse func from MotorSpeedControl!");
    let func_type: &str = parseString(func.get("type"));
    match func_type {
        "constant" => {
            return FuncTypes::ConstFunc { c: parsef32(func.get("c")) }
        },
        "linear" => {
        ...
        _ => {
            error!("ReadInstructions: Function type {} unknown!", func_type);
            process::exit(0);
        }
    }
}
...
fn matchEvent (event: &serde_json::Value, event_list: &mut Vec<Event>) {
    ...
    match event_type {
        ...
        "SensorValue" => {
            event_list.push(
                Event::SensorValue { 
                    ...
                    expr: parseString(event.get("expr")).chars().nth(0).expect("ReadInstructions: Cannot parse char from SensorValue!")
                }
            )
        },
        ...
        _ => {
            error!("ReadInstructions: Event type {} unknown!", event_type);
        }
    }
}
...
fn matchCond(condition: &serde_json::Value, list: &mut Vec<Condition>) {
    ...
    match condition_type {
        ...
        _ => {
            error!("ReadInstructions: Condition type {} unknown!", condition_type);
        }
    }
}

pub fn ReadInstructions(file_path: &str, spawn_list: &mut Vec<Condition>, event_list: &mut Vec<Event>, term_list: &mut Vec<Condition>) {
    ...
    for i in 0..event_list_len {
        let event: &serde_json::Value = json["EventList"].get(i as usize).expect("ReadInstructions: Event at index not available");

        matchEvent(event, event_list);
    }

    for i in 0..spawn_list_len {
        let condition: &serde_json::Value = json["SpawnList"].get(i as usize).expect("ReadInstructions: Spawn condition at index not available");
        matchCond(condition, spawn_list);   
    }
    
    for i in 0..term_list_len {
        let condition: &serde_json::Value = json["TermList"].get(i as usize).expect("ReadInstructions: Term condition at index not available");
