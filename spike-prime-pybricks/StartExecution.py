from consts import *
from ProcessLoop import process_loop
from Port import prepare_motors_sensor, set_speed_pid
from Logger import log_header_csv, init_logger
from ReadInstructions import read_instructions
...
def start_execution(hub, round_instructions_path, port_definitions, active_table, terminated_table, cond_table, notetaker):
    ...
    writer = None
    if DEBUG:
        writer = init_logger(f"records/{name}.csv")
        log_header_csv(writer)  # Assume log_header_csv is defined

    # Prepare motors and sensors
    motors_sensors = prepare_motors_sensor(hub, port_definitions)
